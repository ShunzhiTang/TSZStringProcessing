...
{
    if (TSZStringIsNull(sourceString)) {
        return NO;
    }else{
...
{
    if (TSZStringIsNull(sourceString) || TSZStringIsNull(regexString) ) {
        return NO;
    }
...
+ (BOOL)stringIsValidateEmailAddress:(NSString *)string
{
    ...
    return [TSZStringUitil sourceString:string regexMatch:emailRegex];
}
...
+ (BOOL)stringISValidateMobilePhone:(NSString *)string
{
    ...
    return [TSZStringUitil sourceString:string regexMatch:phoneRegex];
}

+ (BOOL)stringISValidateCarNumber:(NSString *)string
{
    NSString *carRegex = @"^[\u4e00-\u9fa5]{1}[a-zA-Z]{1}[a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-\u9fa5]$";
    return [TSZStringUitil sourceString:string regexMatch:carRegex];
}

+ (BOOL)stringIsValidateUrl:(NSString *)string
{
    NSString *checkRegex = @"^(([hH][tT]{2}[pP][sS]?)|([fF][tT][pP]))\\:\\/\\/[wW]{3}\\.[\\w-]+\\.\\w{2,4}(\\/.*)?$";
    return [TSZStringUitil sourceString:string regexMatch:checkRegex];
}
...
+ (BOOL)stringISValidatePersonCardNumber:(NSString *)string
{
    ...
    return [TSZStringUitil sourceString:string regexMatch:regex2];
}
...
+ (BOOL)stringIsValidateMailCode:(NSString *)string
{
    ...
    return [TSZStringUitil sourceString:string regexMatch:mailCodeRegex];
}
...
+ (NSString *)encodeStringFromDict:(NSDictionary *)dict
{
    ...
    for (id key in dict) {
        @autoreleasepool {
            ...
            NSString *part = [NSString stringWithFormat: @"%@=%@",[TSZStringUitil urlEncode:key],[TSZStringUitil urlEncode:value]];
